...
class UIBuilder(object):
    ...
    def build(self, target: str = "anki21", pyenv: Optional[str] = None):
        if target != "anki21":
    ...
    def _pyenv_prefix(self, pyenv: str) -> str:
        return (
            '''eval "$(pyenv init -)"'''
    ...
    def _write_init_file(self, modules: list, path_out: Path):
    ...
    def _generate_all_str(self, modules: list) -> str:
        module_string = ",\n".join('    "{}"'.format(m) for m in modules)
        out = _template_all.format(module_string)
        return out

    def _generate_import_str(self, modules: list) -> str:
        out = "\n".join("from . import {}".format(m) for m in modules)
        return out

    def _munge_form(self, path: Path):
        """
